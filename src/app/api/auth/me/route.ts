...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (decoded.isGuest) return NextResponse.json({ id: decoded.id, username: decoded.username, isGuest: true, role: 'guest', requestCount: 0 })
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const decoded: any = await verifyJWT(token)
    if (decoded.isGuest) return NextResponse.json({ message: 'Guest cannot update' }, { status: 403 })
